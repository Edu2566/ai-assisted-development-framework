---
name: command-speckit-fix
description: |
  Implements the `/speckit.fix` slash-style command for Codex. Use when the user types `/speckit.fix`, asks to run the `speckit.fix` command, or wants the AI Development Framework workflow: Quick-fix bypass for trivial changes that skip the full SDD workflow.
---
...
- Treat `/speckit.fix` as a command invocation, not as ordinary prose.
- If the user provides text after `/speckit.fix`, treat it as this command argument: `<description of trivial change>`.
...
### Change scope
!`~/.codex/hooks/speckit-helper.sh trivial-change-check`
