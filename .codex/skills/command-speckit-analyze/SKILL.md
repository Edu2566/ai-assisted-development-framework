---
name: command-speckit-analyze
description: |
  Implements the `/speckit.analyze` slash-style command for Codex. Use when the user types `/speckit.analyze`, asks to run the `speckit.analyze` command, or wants the AI Development Framework workflow: Read-only cross-artifact consistency analysis with coverage mapping.
---
...
- Treat `/speckit.analyze` as a command invocation, not as ordinary prose.
- If the user provides text after `/speckit.analyze`, treat it as command input and apply it wherever the recipe asks for user intent.
...
### Load all artifacts
!`~/.codex/hooks/speckit-helper.sh all-artifacts`

### Constitution
!`~/.codex/hooks/speckit-helper.sh constitution`

### Checklists
!`~/.codex/hooks/speckit-helper.sh checklists-content`
