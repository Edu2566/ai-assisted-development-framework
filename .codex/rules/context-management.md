...
mid-turn. Use the context-usage percentage shown in the Codex CLI header as the
signal:
...
Write a progress file (`.codex/progress.md` or project-specific location) containing:
...
- Single root-level `AGENTS.md` is sufficient
- No subagents needed — use main conversation for everything
- Skip explicit plan tracking for tasks with <3 steps
...
- Add directory-specific `AGENTS.md` in complex subdirectories (e.g., `src/api/AGENTS.md`)
- Use subagents (Explore) for research while implementing in main conversation
- Codex plan tracking recommended for all multi-step work
...
- Per-service `AGENTS.md` files with service-specific conventions
...
- Each service may have its own `AGENTS.md` with service-specific build commands and conventions
