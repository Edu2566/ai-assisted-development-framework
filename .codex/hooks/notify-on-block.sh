...
STAMP_FILE="/tmp/.codex-notify-stamp"
NOW=$(date +%s)
...
if command -v notify-send &>/dev/null; then
  # Linux (libnotify)
  notify-send "Codex" "$MESSAGE" --urgency=normal 2>/dev/null || true
elif command -v osascript &>/dev/null; then
  # macOS
  osascript -e "display notification \"$MESSAGE\" with title \"Codex\"" 2>/dev/null || true
fi
