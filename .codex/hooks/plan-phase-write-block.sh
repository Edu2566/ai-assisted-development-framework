...
INPUT=$(cat)
FILE_PATH=$(echo "$INPUT" | jq -r '.tool_input.file_path // .toolInput.file_path // .tool_input.path // .toolInput.path // .file_path // .filePath // empty')
CWD=$(echo "$INPUT" | jq -r '.cwd // .codex_cwd // .working_directory // .workingDirectory // empty')
...
echo "  Finish plan.md, then run:  ~/.codex/hooks/speckit-helper.sh plan-phase-end" >&2
